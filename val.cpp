 ...
 * In "val.h", similarly use class Expr; to allow Val::to_expr's return type to be Expr*. In "val.cpp", use #include "expr.h" in addition to #include "val.h".
 ...
 * If you're using Xcode, don't forget to add "val.cpp" to your test target as well as your main target.
 * To full test a method like NumVal::equals, you'll need a pointer of type Val* that is not a pointer to a NumVal object — but NumVal is the only instantiable Val class at first! To work around that problem, use nullptr in tests for now (even though that's usually a bad idea), since nullptr can have type Val* and is not a pointer to a NumVal object.
 * To deal with adding values in AddExpr::interp, see the slides for a recommendation on adding a method Val::add_to.
